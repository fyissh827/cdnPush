...
router.route('/upload/profilepic').post(userMiddleware.isLoggedIn, picval2.upload.fields([{ name: 'avatarshort', maxCount: 1 }, { name: 'avatar', maxCount: 1 }]),  pic_update.profilepic);
...
router.route('/p/grewtale').post(userMiddleware.isLoggedIn, multi.upload.fields([{ name: 'photos', maxCount: 50 }, { name: 'videos', maxCount: 10 }]), grewtale.grewtale);
router.route('/p/grewtaleplus').post(userMiddleware.isLoggedIn, multi.upload.fields([{ name: 'photos', maxCount: 50 }, { name: 'videos', maxCount: 10 }]), grewtaleplus.grewtaleplus);

//comment +m

router.use('/api/comment/media/v1/photo',  express.static('media/comment/images'));
router.route('/p/comment').post(userMiddleware.isLoggedIn, multi2.upload.array('commentfile', 30), comment.comment);
...
router.route('/chat/message/put').post(userMiddleware.isLoggedIn, multi4.upload.array('file', 30), chat_put.chat_put);
