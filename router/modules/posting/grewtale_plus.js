...
const dimensiondata = JSON.parse(req.body.dimensiondata);
    ...
    for (var i = 0; i < files.length; i++){
		...
		files_store.push({name : '', t : '', e2 : [], e : [], p : []});
		 if(type == 1){
			...
			files_store[i].name = files[i].filename + dimensiondata[i].d;
			files_store[i].t = 2;
			files_store[i].p.push({name :files[i + 1].filename + dimensiondata[i + 1].d, t : 1});
			files.splice(i + 1 , 1);
 ...
 //console.log(obj, file_type, accelerator, userId, wn,  primitive_id, title,  content, standard, privacy);
  axios.post('http://localhost:3000/p/grewtaleplus', { obj, accelerator, file_type, userId, wn,  primitive_id, title,  content, standard, privacy, webMeta})
  .then(response => {
