 ...
 module.exports = { 
    ...
    urlHashing : (a, b) => {
      ...
      for(var i = 0; i < d.length; i++){
        ...
        var a_link =  t[0] + `?` + r + `en_session=` + session + `&u_id=` + b + `&en_date=` + hash  + `&en_name=` + encodeURIComponent(btoa(t[0])); 
       d[i].name = a_link;
       ...
       for(var j = 0; j < d[i].p.length; j++){
        ...
        var a_link2 =  t2[0] + `?` + r + `en_session=` + session + `&u_id=` + b + `&en_date=` + hash + `&en_name=` + encodeURIComponent(btoa(t2[0])); 
    ...
    hashingsingle : (a, b) => {
      ...
      const date = JSON.stringify(Date.now());
       ...
       var encrypted = CryptoJS.AES.encrypt(date, "Secret Passphrase");
       var hash = encodeURIComponent(encrypted);
     var final =  a + `?` +  `en_session=` + session + `&u_id=` + b + `&en_date=` + hash +  `&en_name=` + encodeURIComponent(btoa(a)); 
